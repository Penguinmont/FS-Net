"""
FS-Net 修改版 - 添加类别权重支持解决数据不平衡问题

主要修改：
1. _classify 方法添加 class_weights 参数
2. 使用加权交叉熵损失
"""
...
class FSNet(object):
    ...
    def __init__(self, config, batch_data, trainable=True):
        ...
        # ========== 新增：类别权重 ==========
        # 根据你的数据: low=34, mid=2447, high=1703
        # 计算权重: max_count / count，然后归一化
        if hasattr(config, 'class_weights') and config.class_weights is not None:
            ...
            # 默认权重 (针对你的数据)
            # mid(0): 2447 -> 1.0
            # low(1): 34 -> 72.0 (2447/34 ≈ 72)
            # high(2): 1703 -> 1.44 (2447/1703 ≈ 1.44)
            self.class_weights = tf.constant([1.0, 72.0, 1.44], dtype=tf.float32)
        # ====================================
    ...
    def _classify(self, feature):
        ...
        with tf.variable_scope('classify'):
            ...
            # ========== 修改：加权损失 ==========
            # 计算每个样本的权重
            sample_weights = tf.gather(self.class_weights, self.label)
            
            # 计算交叉熵
            loss_per_sample = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=logit, labels=self.label)
            
            # 加权平均
            loss = tf.reduce_sum(loss_per_sample * sample_weights) / tf.reduce_sum(sample_weights)
            # ====================================
